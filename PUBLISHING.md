...
> 書くと決めていないネタ・公開留保がかかったもの・取り下げたものは、リポジトリ外（Stock ノート等）で別管理する（AGENTS.md §2）。
...
判定オントロジー（公開可否の判定基準と文体判定）は非公開の別リポジトリにある。判定基準そのものが自社の弱点の所在を示すためである。
...
| 判断 | 担当 |
|---|---|
| 何を書くか・公開してよいか | CTO（ネタ管理先の観点ルールと、非公開の判定オントロジー） |
